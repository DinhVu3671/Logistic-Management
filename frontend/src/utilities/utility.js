...
const getVehicleType = (code) => {
  return listVehicleType.map((type, index) => {
    if (type.code === code) return type.description;
  });
};
...
const getCategory = (code) => {
  return listProductCategory.map((category, index) => {
    if (category.code === code) return category.description;
  });
};

const getDeliveryMode = (code) => {
  return listDeliveryMode.map((deliveryMode, index) => {
    if (deliveryMode.code === code) return deliveryMode.description;
  });
};
...
const getStatus = (code) => {
  return STATUS_OBJ.map((status, index) => {
    if (status.code === code) return status.title;
  });
};
...
const checkExistNode = (checkedNode, nodes) => {
  for (let index = 0; index < nodes.length; index++) {
    ...
    if (node.code === checkedNode.code) return index;
  }
  return null;
};
...
const checkExistProduct = (checkedProduct, products) => {
  for (let index = 0; index < products.length; index++) {
    ...
    if (product.id === checkedProduct.id) return index;
  }
  return null;
};
...
export {
  ...
  getCategories,
  getProducts,
  getOrdinalLoadingOrders,
  getFormatDate,
  getIconURL,
  getCustomersOfJourney,
  getDepotsOfJourney,
  checkExistNode,
  getListProductOfOrderItems,
  getStatusColor,
  getStatus,
};

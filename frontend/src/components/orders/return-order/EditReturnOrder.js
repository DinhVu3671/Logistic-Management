...
class EditReturnOrder extends Component {
    constructor(props) {
        super(props)
        this.state = {
            order: this.props.order,
            editReturnOrder: this.props.editReturnOrder,
            weight: '',
            capacity: '',
            orderItems: this.props.editReturnOrder.orderItems,
            listProduct: [],
        };
    ...
    componentDidMount() {
        console.log("this.props.editReturnOrder:", this.props.editReturnOrder);
        const productsData = getListProductOfOrderItems(this.state.order.orderItems);
        let orderItems = this.props.editReturnOrder.orderItems;
        orderItems.map(orderItem => {
            orderItem.products = productsData;
        });
        this.setState({
    ...
    handleSubmit = () => {
        ...
        let returnOrderData = {
            id: this.props.editReturnOrder.id,
            weight: 0,
            capacity: 0,
        ...
        returnOrderService.update(this.state.order.id, this.props.editReturnOrder.id, returnOrderData).then(response => {
            const data = response.data;
            if (data.code === 'SUCCESS') {
                this.props.setShowEdit(false);
                this.props.showSuccessMsg("Edit returnOrder " + data.data.id + " successfully");
                        ...
                        <CCard>
                            ...
                            <CCardBody>
                                ...
                                <CFormGroup className="my-0">
                                    {this.state.isReadyEdit && this.createReturnOrderItems()}
                                </CFormGroup>
                            </CCardBody>
                        </CCard>
                    </CModalBody>
